...
### **AI / RAG Pipeline**
*   **AI Service:** Python FastAPI (service riêng, port 8000) — Node.js chỉ đóng vai trò proxy request
*   **LLM API:** Google Gemini API (Gemini-2.5-flash & Gemini-2.0-flash)
*   **Orchestrator:** LangChain (Python) + Motor (async MongoDB driver)
*   **Vector Database:** MongoDB Atlas Vector Search
*   **Document Parsers:** Pdf-parse, Mammoth (cho DOCX), XLSX

## ⚠️ Giới hạn hệ thống (Limitations)

*   **Knowledge base:** Cần upload tài liệu thủ công qua trang Admin → AI Knowledge để chatbot có ngữ cảnh y khoa
*   **Gemini API:** Chatbot yêu cầu `GEMINI_API_KEY` hợp lệ — không hoạt động offline hoặc khi hết quota
*   **Timezone:** Hệ thống cố định múi giờ Việt Nam (UTC+7) — không hỗ trợ đa múi giờ
*   **Rate limit chatbot:** 20 request/phút cho public chatbot endpoint (không đăng nhập)
...
> Screenshots thực tế xem tại thư mục `/docs/screenshots/` hoặc chạy ứng dụng và khám phá trực tiếp.
